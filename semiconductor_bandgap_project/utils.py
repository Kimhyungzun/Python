...
def plot_act_vs_pred(y_actual, y_predicted):
    ...
    plt.plot(y_actual, y_predicted, marker='o', markersize=18, mfc='#0077be', color='k', linestyle='none', alpha=0.6)
    ...
    plt.xlim(limits)
    plt.ylim(limits)
    plt.xticks(size=text_size-4)
    plt.yticks(size=text_size-4)
    plt.xlabel('Actual[eV]', size=text_size)
    plt.ylabel('Predicted[eV]', size=text_size)
